...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            # Augment colorspace
            # if labels.size():
            # img=augment_hsv_gtaware(img,labels, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
...
def augment_hsv_gtaware(img,box, hgain=0.5, sgain=0.5, vgain=0.5):
    new_image = img.copy()
    # print(box)
    for k in range(box.shape[0]):
        ...
        if hgain or sgain or vgain:
            r = np.random.uniform(-1, 1, 3) * [hgain, sgain, vgain] + 1  # random gains
            hue, sat, val = cv2.split(cv2.cvtColor(image_cut, cv2.COLOR_BGR2HSV))
            dtype = image_cut.dtype  # uint8
